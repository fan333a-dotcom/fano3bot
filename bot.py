...
def get_prompt(user_id: int) -> str:
    ...
    return (
        "أنت بوت تلغرام زنخ وثقيل دم ووقح. تتحدث باللهجة الفلسطينية البدوية. "
        "ردودك قصيرة وساخرة ومزعجة. "
        "ما عندك أدب مع الناس، ترد بكل برود واستفزاز. "
        "استخدم كلمة 'تقرفنييش' و'تقرفوناش' بشكل طبيعي. "
        "لا تتكلم عن الوطن أو الهجرة إلا إذا سألك الشخص عنها بشكل مباشر. "
        "لا تصرح بهويتك."
    )
...
def main_handler(message):
    ...
    if text == "الألعاب" or text == "الالعاب" or text == "قائمة الألعاب":
        menu = (
            "🎮 **قائمة الألعاب:**\n\n"
            "🔹 **لو خيروك** — خيارات صعبة 🤔\n"
            "🔹 **كت تويت** — أسئلة للشات 💬\n"
            "🔹 **نكتة** — ضحك ونكت 🤣\n"
            "🔹 **فعالية** — نشاط جماعي ✨\n"
            "🔹 **صراحة** — سؤال صراحة 🤫\n"
            "🔹 **مثل** — حكمة ومَثَل 📜\n"
            "🔹 **نسبة الحب** — قياس المحبة 🥰\n"
            "🔹 **معلوماتي** — نقاطك ورتبتك 📊"
        )
        bot.reply_to(message, menu)

    if text == "الاوامر" or text == "قائمة الأوامر":
        menu = (
            "📋 **قائمة أوامر فنوع**\n\n"
            "🎯 **عام:**\n"
            "`الرابط` — رابط الجروب\n"
